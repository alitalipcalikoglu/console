...
/** @typedef {{ id: string, type: 'notify'|'auth'|'media'|'gateway'|'audit'|'shortlink'|'flags'|'scheduler'|'webhook-out'|'search'|'ratelimit'|'geo', label: string, url: string, publicUrl: string|null, hasMetrics: boolean, polling: Polling }} ServiceInfo */
...
export class Services {
  ...
  static ICONS = { notify: 'bell', auth: 'users', media: 'image', gateway: 'route', audit: 'history', shortlink: 'link', flags: 'flag', scheduler: 'clock', 'webhook-out': 'webhook', search: 'search', ratelimit: 'gauge', geo: 'globe' };
