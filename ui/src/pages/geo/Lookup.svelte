<script>
  ...
  async function create(body) {
    creating = true;
    try { const r = /** @type {any} */ (await api.post(`${base}/collections`, body)); toasts.ok(t('ge.created', { name: r.collection.name })); createOpen = false; router.go(`/geo/${sid}/collections/${r.collection.name}`); }
          ...
          <div class="table-wrap"><table class="table">
            ...
            <tbody>
              {#each items as c (c.name)}
                <tr class="clickable" onclick={() => router.go(`/geo/${sid}/collections/${c.name}`)}>
