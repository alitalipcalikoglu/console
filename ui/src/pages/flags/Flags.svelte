<script>
  ...
  let archived = $state(router.query.get('archived') === 'true');
  let q = $state(router.query.get('q') ?? '');
  let tag = $state(router.query.get('tag') ?? '');
  let kind = $state(router.query.get('kind') ?? '');
  ...
  $effect(() => { archived; q; tag; kind; clearTimeout(timer); timer = setTimeout(() => { router.setQuery({ archived: archived ? 'true' : null, q, tag, kind }); list.load(); }, 300); return () => clearTimeout(timer); });
  $effect(() => { stats.load(); });
  ...
  async function create(body) {
    creating = true;
    try { const r = /** @type {any} */ (await api.post(`/services/${sid}/flags/flags`, body)); toasts.ok(t('fl.created', { key: r.flag.key })); createOpen = false; router.go(`/flags/${sid}/flags/${r.flag.key}`); }
      ...
      <div class="table-wrap"><table class="table">
        ...
        <tbody>
          {#each items as f (f.key)}
            <tr class="clickable" onclick={() => router.go(`/flags/${sid}/flags/${f.key}`)}>
