<script>
  ...
  async function create(body) {
    creating = true;
    try { const r = /** @type {any} */ (await api.post(`${base}/policies`, body)); toasts.ok(t('rl.created', { name: r.policy.name })); createOpen = false; router.go(`/ratelimit/${sid}/policies/${r.policy.name}`); }
      ...
      <div class="table-wrap"><table class="table">
        ...
        <tbody>
          {#each items as x (x.name)}
            <tr class="clickable" onclick={() => router.go(`/ratelimit/${sid}/policies/${x.name}`)}>
