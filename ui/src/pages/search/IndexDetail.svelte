<script>
  ...
  let sort = $state(router.query.get('sort') ?? 'relevance');
  /** @type {Record<string, string[]>} */ let filters = $state(JSON.parse(router.query.get('f') ?? '{}'));
  ...
  $effect(() => { q; sort; filters; x; clearTimeout(timer); timer = setTimeout(() => { router.setQuery({ q, sort: sort === 'relevance' ? null : sort, f: Object.keys(filters).length ? JSON.stringify(filters) : null }); search(0); }, 250); return () => clearTimeout(timer); });
...
<Confirm open={confirm === 'delete'} title={t('se.delete')} message={t('se.deleteDesc')} danger typeWord={x?.name} confirmLabel={t('common.delete')} busy={busy} onconfirm={() => run(async () => { await api.delete(base); router.go(`/search/${sid}`); }, t('se.deleted'))} oncancel={() => { confirm = null; }} />
