<script>
  ...
  let enabled = $state(router.query.get('enabled') ?? '');
  let q = $state(router.query.get('q') ?? '');
  let tag = $state(router.query.get('tag') ?? '');
  let runStatus = $state(router.query.get('status') ?? '');
  ...
  $effect(() => { enabled; q; tag; clearTimeout(timer); timer = setTimeout(() => { router.setQuery({ enabled, q, tag }); list.load(); }, 300); return () => clearTimeout(timer); });
  $effect(() => { runStatus; router.setQuery({ status: runStatus }); runs.load(); });
  $effect(() => { stats.load(); });
  ...
  async function create(body) {
    creating = true;
    try { const r = /** @type {any} */ (await api.post(`/services/${sid}/scheduler/jobs`, body)); toasts.ok(t('sc.created', { name: r.job.name })); createOpen = false; router.go(`/scheduler/${sid}/jobs/${r.job.name}`); }
        ...
        <div class="table-wrap"><table class="table">
          ...
          <tbody>
            {#each items as j (j.name)}
              <tr class="clickable" onclick={() => router.go(`/scheduler/${sid}/jobs/${j.name}`)}>
