<script>
  ...
  import { fetchDocServices, fetchOpenApi } from '../lib/api-docs.js';
  ...
  /** @type {DocService[]} */
  let items = $state([]);
  let selected = $state(router.query.get('service') ?? 'console');
  ...
  function choose(/** @type {Event} */ event) {
    ...
    router.setQuery({ service: selected === 'console' ? null : selected });
    loadDocument();
  }
