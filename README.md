...
One installable web app to watch and operate the atc-web services: notify, auth, media, gateway, audit, shortlink, flags, scheduler, webhook-out, search and ratelimit. Fastify backend-for-frontend plus a Svelte 5 progressive web app. Works on phones and desktops, light and dark, Turkish and English.
...
`type` is one of `notify`, `auth`, `media`, `gateway`, `audit`, `shortlink`, `flags`, `scheduler`, `webhook-out`, `search`, `ratelimit` and selects the screens and endpoints; several instances of one type are allowed (tabs appear). `url` is the internal address the console calls; `publicUrl` is optional information for auth. The gateway needs `metricsTokenEnv` instead of an API key. The file is validated at startup with precise error messages. Changing connections means a restart (`pm2 reload console`).
