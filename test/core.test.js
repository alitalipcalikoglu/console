...
test('AdminStore: create, lookup, lockout, TOTP enrolment flags, admin count', () => {
  const admins = new AdminStore(testDb());
...
test('ServiceRegistry parses, resolves secrets, rejects broken files', () => {
  ...
  assert.deepEqual(r.services.map((s) => s.id), ['notify', 'auth', 'media', 'gateway', 'audit', 'shortlink', 'flags', 'scheduler', 'webhooks', 'search', 'ratelimit']);
