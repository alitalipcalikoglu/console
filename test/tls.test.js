...
const dir = mkdtempSync(join(tmpdir(), 'console-tls-'));
...
test('serves HTTPS with HSTS when TLS_CERT_PATH and TLS_KEY_PATH are set', async () => {
  ...
  writeFileSync(join(dir, 'index.html'), '<title>c</title>');
  const t = await testConsole({ env: { TLS_CERT_PATH: cert, TLS_KEY_PATH: key }, publicDir: dir });
  await t.app.listen({ port: 0, host: '127.0.0.1' });
  try {
    const addr = /** @type {import('node:net').AddressInfo} */ (t.app.server.address());
