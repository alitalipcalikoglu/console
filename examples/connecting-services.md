...
- `type`: `notify`, `auth`, `media`, `gateway` or `audit`. Decides the screens.
...
For the audit service use a **read** role key (`AUDIT_API_KEYS=…,console:<secret>:read`): the console only reads events, verifies the chain and exports; it never writes there.
