<script>
  ...
  let f = $state(Object.fromEntries(KEYS.map((k) => [k, route.url.searchParams.get(k) ?? ''])));
  ...
  $effect(() => {
    ...
    clearTimeout(timer);
    timer = setTimeout(() => { Navigation.replaceQuery(route.url, Object.fromEntries(KEYS.map((k) => [k, f[k] || null]))); list.load(); }, snapshot ? 300 : 0);
        ...
        <div class="table-wrap"><table class="table">
          ...
          <tbody>
            {#each items as e (e.id)}
              <tr class="clickable" onclick={() => goto(`/audit/${sid}/events/${e.id}`)}>
