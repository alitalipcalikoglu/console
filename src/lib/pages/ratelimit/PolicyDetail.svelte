<script>
  ...
  // ---- hourly decisions
  let hours = $state(Number(route.url.searchParams.get('h') ?? 24));
  ...
  // ---- subject lookup
  let lookupInput = $state(route.url.searchParams.get('s') ?? '');
  let subject = $state(route.url.searchParams.get('s') ?? '');
  const usage = new Resource(() => api.get(`${base}/subjects/${encodeURIComponent(subject)}`));
  const u = $derived(/** @type {any} */ (usage.data));
  function lookup() { const s = lookupInput.trim(); if (!s) return; subject = s; Navigation.replaceQuery(route.url, { s }); usage.load(); }
  /** @param {string} s */
  function pick(s) { lookupInput = s; lookup(); }

  $effect(() => { res.load(); loadOverrides(0); top.load(); });
  $effect(() => { hours; Navigation.replaceQuery(route.url, { h: hours === 24 ? null : String(hours) }); stats.load(); });
...
<Confirm open={confirm === 'delete'} title={t('rl.delete')} message={t('rl.deleteDesc')} danger typeWord={x?.name} confirmLabel={t('common.delete')} busy={busy} onconfirm={() => run(async () => { await api.delete(base); goto(`/ratelimit/${sid}`); }, t('rl.deleted'))} oncancel={() => { confirm = null; }} />
