<script>
  ...
  $effect(() => { status; q; tag; clearTimeout(timer); timer = setTimeout(() => { Navigation.replaceQuery(route.url, { status, q, tag }); list.load(); }, 300); return () => clearTimeout(timer); });
  ...
  async function create(body) {
    busy = true;
    try { const r = /** @type {any} */ (await api.post(`/services/${sid}/shortlink/links`, body)); toasts.ok(t('sl.created', { code: r.link.code })); createOpen = false; goto(`/shortlink/${sid}/links/${r.link.code}`); }
        ...
        <div class="table-wrap"><table class="table">
          ...
          <tbody>
            {#each items as l (l.code)}
              <tr class="clickable" onclick={() => goto(`/shortlink/${sid}/links/${l.code}`)}>
